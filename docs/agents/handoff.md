...
- **Remaining council steps:** a user-selectable arrival reserve (make `AUTO_MIN_ARRIVAL_PCT` an atom used by the projection, reserve lines and optimizer payload); a battery-along-route chart from `projectTripCharging` data; move `EnergySimulationPanel` to `/research/energy`; the evaluation harness.
