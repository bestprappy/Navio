...
7. Prefer reusable UI widgets over page-section extraction:
...
8. Follow SOLID principles strictly:
...
9. Prioritize optimization:
   - Minimize re-renders.
   - Memoize expensive computations.
   - Use stable props and callbacks.
   - Avoid unnecessary client-side state.
   - Keep bundle size small and split when needed.
10. Stick with Jotai for UI state (atoms/selectors/hooks) and avoid introducing other global client-state libraries unless explicitly requested.
11. Use TanStack Query for all API calls, server-state caching, invalidation, and async loading/error handling.
12. In Next.js App Router, any component that uses React Context, Jotai hooks, event handlers, or browser APIs must be a Client Component with "use client" at the top of the file.
13. Keep Server Components for server-rendered composition and data boundaries; do not use React Context consumers/providers in Server Components.
14. Every compound child that consumes context must use a typed custom hook (for example useSidebarContext) that throws a clear, descriptive error when used outside its parent provider.
15. Enforce accessibility for compound widgets: semantic roles/ARIA attributes, focus management, visible focus states, and keyboard support (Tab/Shift+Tab, Escape, Arrow keys, Enter/Space) where applicable.
16. If mock data is needed, always place it in data.ts (typed, exported, reusable).
17. Follow `.claude/rules/client/styling-guide.md` for all styling decisions.
18. Do not use any; define clear TypeScript types/interfaces.
19. Keep each file focused and production-ready.
...
14. Provide final folder tree and complete code.
