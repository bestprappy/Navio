...
11. For Spring Boot configuration, always use `.yml` files (for example `application.yml`) instead of `.properties` files.
...
8. Spring Boot configuration uses `.yml` files only; `.properties` files are not introduced.
